...
预期：抓取真实新闻成功（网络可用时）→ 热点分析报"未配置 API Key"错误被捕获 →
话题卡同样报错 → 整体返回 ok=False + errors，不崩溃、不产生脏数据。
"""
import sys, io
sys.path.insert(0, ".")
...
r = pipe.run_daily()
print("ok:", r.get("ok"))
print("hotspots:", r.get("hotspots"), "| cards:", r.get("cards"))
print("errors:", len(r.get("errors", [])))
for e in r.get("errors", [])[:4]:
    print("  -", e[:90])

# 断言：无 key 时任何 AI 生成项都为 0，且错误信息包含"API Key"
assert r.get("hotspots") == 0 and r.get("cards") == 0, r
assert any("API Key" in e for e in r.get("errors", [])), r.get("errors")
# 且没有写入任何草稿脏数据
ov = store.overview()
assert ov["hotspots_draft"] == 0 and ov["cards_draft"] == 0, ov
